...
Outputs:
  - users_flat.csv        (one row per user; all available user properties flattened)
...
Optional:
  ...
  USER_SELECT   ("*" for all fields via per-user refetch, or comma list for $select; default is a sane list)
  REFETCH_FULL  ("1" to force per-user refetch with full objects; default auto-on when USER_SELECT="*")
...
import os, json, time, base64, random, logging, urllib.parse, csv
...
# ----------------------------
#TENANT_ID     = os.getenv("AZ_TENANT_ID", "").strip()
#CLIENT_ID     = os.getenv("AZ_CLIENT_ID", "").strip()
#CLIENT_SECRET = os.getenv("AZ_CLIENT_SECRET", "").strip()
...
# Projection controls
ENV_USER_SELECT = os.getenv("USER_SELECT", "").strip()  # "*" or csv list or empty
# Default REFETCH_FULL to on when USER_SELECT="*"
REFETCH_FULL = os.getenv("REFETCH_FULL", "1" if ENV_USER_SELECT == "*" else "0").strip() == "1"

# Default fields if USER_SELECT is not provided
...
def graph_get(s: requests.Session, url: str, token: str) -> requests.Response:
    ...
    return with_backoff(s.get, url, headers=headers)
...
# ----------------------------
# Helpers: build list URL according to USER_SELECT
# ----------------------------
def _build_users_list_url() -> str:
    """
    Build the /users list URL.
    - Uses $top up to Graph's practical maximum (999) so we page less.
    - Adds $count=true (requires ConsistencyLevel: eventual header) so we can see totals if desired.
    - USER_SELECT:
        "*"  -> no $select (default projection from list), still all rows via paging
        csv  -> use those fields in $select
        ""   -> use DEFAULT_USER_SELECT_FIELDS
    """
    qs = []

    if ENV_USER_SELECT and ENV_USER_SELECT != "*":
        qs.append("$select=" + ENV_USER_SELECT)
    elif not ENV_USER_SELECT:
        qs.append("$select=" + ",".join(DEFAULT_USER_SELECT_FIELDS))

    qs.append("$top=" + str(min(limiter.page_size, 999)))
    qs.append("$count=true")

    if USER_FILTER:
        qs.append("$filter=" + urllib.parse.quote(USER_FILTER, safe=" =()'\""))

    return f"{GRAPH_ROOT}/users?{'&'.join(qs)}"

# ----------------------------
# Fetch users (paged list)
# ----------------------------
def fetch_all_users(token: str) -> List[Dict[str, Any]]:
    url = _build_users_list_url()
    ...
    with requests.Session() as s:
        while url:
            ...
            log.info(
                f"Fetched users: {len(users)} so far "
                f"(page_sleep={limiter.page_sleep:.2f}s, top={limiter.page_size})"
            )
            if url:
...
# ----------------------------
# Optional: per-user full refetch via $batch (no $select)
# ----------------------------
def batch_refetch_full_users(token: str, users: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Refetch each user (GET /users/{id}) to obtain the full object as returned by the single-resource endpoint.
    Merges fields into the existing user dicts.
    """
    if not users:
        return users

    id_to_user = {u["id"]: u for u in users if "id" in u}
    pending = list(id_to_user.keys())

    with requests.Session() as s:
        while pending:
            chunk_ids = pending[:BATCH_LIMIT]
            pending = pending[BATCH_LIMIT:]

            body = {
                "requests": [
                    {"id": str(i+1), "method": "GET", "url": f"/users/{uid}"}
            ...
            for item in responses:
                ...
                if status == 200 and isinstance(body, dict):
                    # Merge/overwrite into the original user object
                    id_to_user[uid].update(body)
                    ...
                    log.warning(f"Refetch full user failed uid={uid} status={status} body={short}")
...
def build_hierarchy(users: List[Dict[str, Any]], manager_map: Dict[str, Optional[str]]):
    ...
    # Flat viewer array (kept minimal)
...
# ----------------------------
# CSV writer (flatten dicts/lists to JSON strings)
# ----------------------------
def write_users_csv(users: List[Dict[str, Any]], path: str):
    # Gather all keys across users
    all_keys = set()
    for u in users:
        all_keys.update(u.keys())

    # Prefer some columns first (if present), then the rest sorted
    preferred = ["id", "displayName", "userPrincipalName", "mail", "jobTitle", "department", "managerId"]
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for u in users:
            row = {k: clean(u.get(k, "")) for k in ordered}
            writer.writerow(row)
...
def main():
    ...
    if not users:
        ...
        # Still write empty JSONs for viewer/tree for consistency
        for p in (OUT_TREE_FILE, OUT_VIEWER_FILE):
            with open(p, "w", encoding="utf-8") as f: json.dump([], f, indent=2, ensure_ascii=False)
        # And empty CSV
        write_users_csv([], USERS_CSV_FILE)
        return

    # If caller wants truly "all fields":
    #  - set USER_SELECT="*" (list gives default set; we then REFETCH_FULL to ensure per-user full object)
    #  - or set REFETCH_FULL=1 to force per-user full object
    if ENV_USER_SELECT == "*" or REFETCH_FULL:
        log.info("Refetching full user objects via $batch …")
        users = batch_refetch_full_users(token, users)
    ...
    # Write CSV for users (all properties)
    write_users_csv(users, USERS_CSV_FILE)
    log.info(f"✅ Wrote {USERS_CSV_FILE}  [{len(users)} users]")

    # Write JSONs for viewer/tree (optional but handy)
